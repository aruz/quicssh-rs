...
async fn handle_connection(proxy_for: SocketAddr, connection: quinn::Connection) {
    ...
    let mut ssh_conn = match ssh_stream {
        Ok(conn) => conn,
        Err(e) => {
    ...
    let mut quic = tokio::io::join(quinn_recv, quinn_send);

    match tokio::io::copy_bidirectional(&mut ssh_conn, &mut quic).await {
        Ok((up, down)) => debug!("[server] bridge closed; up={}B down={}B", up, down),
        Err(e) => warn!("[server] bridge error: {}", e),
    }
